...
For nField := 1 To (cAliasCP)->(FCount())
	...
	If (cAlias)->(FieldPos(cField)) > 0 .And. (cAliasCP)->(FieldPos(cField)) > 0 .And. ValType(&(cAliasCp + "->" + cField)) <> "M"
		xValue := &(cAliasCp + "->" + cField)
		IF mv_par04 = 1 .And. AllTrim(cField) $ "X3_USADO,X3_RESERV,X3_OBRIGAT"
			If SX3->X3_GRPSXG = "033" .And. AllTrim(cField) == "X3_USADO" 	// Campo Filial
				xValue := "x       x       x       x       x       x       x       x       x       x       x       x       x       x       x"
			ElseIf SX3->X3_GRPSXG = "033" .And. AllTrim(cField)$ "X3_RESERV,X3_OBRIGAT" 	// Campo Filial
				xValue := ""
			ElseIf AllTrim(cField) == "X3_USADO"
				// A1_DDD (Campo N„o Usado)
				xValue := "x       x       x       x       x       x       x       x       x       x       x       x       x       x       x x"
				If X3USO(NEW->X3_USADO)
					// A1_NOME (Campo Usado)
					xValue := "x       x       x       x       x       x       x       x       x       x       x       x       x       x       x x"
				EndIf
			ElseIf AllTrim(cField) == "X3_RESERV"
				xValue := " x     x"
				If X3OBRIGAT(SX3->X3_CAMPO)
					xValue := "xxxxxx x"
				ElseIf X3USO(NEW->X3_USADO)
					xValue := "  x  xx"
				EndIf
			ElseIf AllTrim(cField) == "X3_OBRIGAT"
				xValue := ""
			EndIf
		EndIf
...
Local nDicDB		:= 1
	...
	aAdd(_aParam ,{3, "Dicionario Banco",nDicDB,{ "Sim", "N„o" },70,,.F.})
